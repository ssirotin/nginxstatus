...
if [[ $nginxstatus == "running" ]]
  then
    ...
    $tg "web server NGINX is running $(curl -sI 192.168.0.13 | head -n 1 | cut -d $" " -f 2,3,4,5,6)"
  else
    echo -en "${WHITE} ${BGRED} web server NGINX not running ${NORMAL}\n"
    $tg "web server NGINX not running $(curl -sI 192.168.0.13 | head -n 1 | cut -d $" " -f 2,3,4,5,6)"
    ...
    $tg "NGINX web server status $(systemctl status nginx | grep -Eo "running|failed|dead") after restart $(curl -sI 192.168.0.13 | head -n 1 | cut -d $" " -f 2,3,4,5,6)"
fi
